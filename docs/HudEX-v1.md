...
| 风格 | 标签 | 悬停卡片 | 额外元素 |
|---|---|---|---|
| 拟物 | 卡片 + 1pt 立体高光/暗边、右侧洞洞 | **标签同色系纸张**：不透明横线 + 细斜纹理、圆角 6、轻微投影 | 洞洞（颜色 = 优先级）+ 曲线连接 |
| 磨砂 | 纯色 | `.regularMaterial` 半透明 | — |
| 液态玻璃 | 纯色、连续圆角 | `.ultraThinMaterial`、紧凑对齐 | — |
...
* 连接线**两端都打洞**：起点在标签的洞洞（标签内部画一小段短线连到边缘，所以看起来是一根线从洞里出来），
  终点是卡片近侧边上的另一个洞洞（`cardHoleCenter`），卡片面板从标签边缘开始，不会盖住标签。
* 线永远是**曲线**：两个控制点都拉到 gap 的 75% 处，再给同样的侧向弯曲（`bow = clamp(0.25 × 弦长, 6…14)`），
  所以即使两个洞在同一水平线上，线也是自然下垂的弧线而不是一根直棍。
