...
| 风格 | 标签 | 悬停卡片 |
|------|------|----------|
| 拟物（默认） | 小卡片，边缘一点点立体感，右侧有一个洞洞 | 不透明的横线纸卡片，洞洞引出一条曲线连到卡片；洞洞颜色 = 项目优先级 |
| 磨砂 | 纯色 | 磨砂半透明，任何背景下文字都清楚 |
| 液态玻璃 | 纯色，圆角连续、和卡片对齐 | 很薄的液态玻璃，紧凑对齐的卡片 |
| 极简 | 只有线框、文字用状态色 | 只有线框，完全透明 |

洞洞只在拟物风格里出现；它的颜色来自项目里的 `优先级：高 / 中 / 低`（也可以写 `priority: high`）。
