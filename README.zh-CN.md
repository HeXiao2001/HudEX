...
**装现成的安装包**（推荐）：下载 `HudEX-1.0.0.pkg` 双击安装，或者用
`HudEX-1.0.0.dmg` 把应用拖进「应用程序」。HudEX 只做了本地（ad-hoc）签名、
没有公证，所以第一次打开会被 macOS 拦一下：在「应用程序」里对 HudEX 点
右键 → 打开 → 再点「打开」。命令行无弹窗安装：
`sudo installer -pkg HudEX-1.0.0.pkg -target /`。
